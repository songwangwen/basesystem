        ...
        $.ajax({
            ...
            error: function (data,b,c) {
                ...
                if(data!=null&&data.responseText=="login"){//需要再次登录
                	...
                	alertDialog("系统出错，请刷新重试，如果仍然无法解决，请联系管理员！", -1);
                }
            }
        });
    } catch (e) {
...
function GetWebControls(element) {
    ...
    var o = {};
    o.jsonEntity = JSON.stringify(jQuery.parseJSON('{' + reVal + '}'));
    var result = {};
    ...
    for (var key in data) {
        ...
        switch (type) {
            ...
            default:
            	//为了兼容chrome，使用attr
            	 	var brow=$.browser;  
		            if(brow.msie) {id.val(value);}  
		            else if(brow.mozilla) {id.val(value);}  
		            else{
		            	id.attr('value',value);
		            }
                break;
 ...
 * @return
 */
function TruncateWebControls(form) {
}
/*
自动给控件赋值、对Lable
*/
...
function closeDialog() {
    ...
    var top = $(window).scrollTop() - wrap[0].offsetHeight;
...
/**
弹出对话框（带：确认按钮、取消按钮）,点击确定关闭
 * @param url 弹出框内容，html路径
 * @param _id 弹出框的ID
 * @param _title 弹出框标题
 * @param _width 弹框宽度
 * @param _height 弹框高度
 * @param callBack 回调函数，点击确定按钮时触发
 * @return
 */
/*
...
@param tablename：表名、
@param fieldName：列名
@param keyFieldName：主键列列名字
@param Msg：提示消息
*/
function FieldExist(id, tablename, fieldName,keyFieldName,Msg) {
	var flag = true;
    var $ObjFiel = $("#" + id);
    var postData = {
        tableName: tablename,		//数据表名
        fieldValue: $ObjFiel.val(), //当前数值
        keyField: GetQuery('KeyValue'), //记录ID,如果存在，则不比较自己
        fieldName: fieldName,			//比较列名字
        keyFieldName: keyFieldName		//列名字
    }
    if(postData.fieldValue!=undefined&&postData.fieldValue!=""){
    	var o = {};
    	o.jsonEntity = JSON.stringify(postData);
    	var result = {};
        result.jsonForm = encodeURIComponent(encodeURIComponent(JSON.stringify(o)));
    	AjaxJson('/utility/fieldExist.json',result,function (data) {
    		if (data>=1) {//已存在
    			tipCss("#" + id, Msg+"["+$ObjFiel.val()+"]已存在,请重新输入");
    			flag = false;
    		}
    		Loading(false);
    	});
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
	...
	if(tabid!="Imain"){
		...
		//$("#navigationtitle").show(300);
		$(".navigation").show(300);
	}*/
    $('#overlay_startmenu').hide(); $('#start_menu_panel').hide(); $('#start_menu_panel .nicescroll-rails').show(); 
    $('.nicescroll-rails').hide();
    if (url == "" || url == "#" || url == RootPath()) {
        url = RootPath() + "/pages/Error/404.html";
    }
    var tabs_container = top.$("#tabs_container");
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span  title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a class='icon " + img + "'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
        	tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span  title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false')\"><a class='icon " + img + "'>" + name + "</a></span></li>");
        }
        ContentPannel.append("<iframe id=\"tabs_iframe_" + tabid + "\" name=\"tabs_iframe_" + tabid + "\" height=\"100%\" width=\"100%\" src=\"" + url + "\" frameBorder=\"0\" style=\"display: block;\"></iframe>");
//          ContentPannel.append("<iframe id=\"tabs_iframe_" + tabid + "\" name=\"tabs_iframe_" + tabid + "\" height=\"100%\" width=\"100%\" src=\"" + url + "\" frameBorder=\"0\" style=\"display: block;width:"+ContentPannel.width()+"px;\"></iframe>");
    } else {
        if (!IsVisitorModule) {
            VisitorModule(tabid, name);
        }
...
function RemoveDiv(obj) {
    ...
    if(tabs_container.find("#tabs_" + obj).attr('id')=='tabs_Imain'){
    	//如果关闭的是首页，忽略命令，直接返回
    	return false;
    }
    ...
    if (tablist.length > 0) {
    	if(tablist[tablist.length - 1].id=='tabs_Imain'){//如果是首页，为了左侧的菜单直接会被加载出来，使用
    		AddTabMenu('Imain','/pages/Home/home.jsp','欢迎首页',false);
    	}else{
...
function resizeU() {
    ...
    //左侧菜单栏位置需要空出，需要设置左边距
    $("#ContentPannel").css("margin-left",$(".navigation").width()+"px");
    $(".ScrollBar").height(divkuangH - 180);
    //投Tab宽度
    $("#dww-menu").width(divkuangW-290);
    $("#dww-menu .mod-hd").width(divkuangW-290);
}
