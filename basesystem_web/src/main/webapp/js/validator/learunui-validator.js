 ...
 * @return
 */
function isRangeLength(obj,min,max){
	var minV = (min==undefined)?0:parseInt(min);
	var maxV = (max==undefined)?99999999:parseInt(max);
	var controlObj = $.trim(obj);
    if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
        return false;
    }
    if(controlObj.length>=minV&&controlObj.length<=maxV){
    	return true;
    }
    return false;
}
    ...
    $(obj).find(".required").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            switch ($(this).attr("checkexpession")) {
                ...
                case "rangelength":
                {
                    if (!isRangeLength(value, $(this).attr("minlength"),$(this).attr("maxlength"))) {
                        Validatemsg = $(this).attr("err");
                        var min = $(this).attr("minlength");
                        var max = $(this).attr("maxlength");
                        if(min!=undefined&&max!=undefined){
                        	Validatemsg+="长度必须介于 "+min+" 至 "+max+"个字符之间！\n";
                        }else if(min!=undefined&&max==undefined){
                        	Validatemsg+="长度必须小于 "+min+"个字符之间！\n";
                        }else if(min==undefined&&max!=undefined){
                        	Validatemsg+="长度必须大于 "+max+"个字符之间！\n";
                        }
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }
